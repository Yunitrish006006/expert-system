...
(deftemplate remove (multislot no))
;; 要移除的
...
;; 打印選不出結果
(defrule print-no-result
  (phase print-no-result)
  =>
  (printout t "Fail:: All candidates are removed" crlf)
  (halt)
)
...
(defrule print-removed-names
  ...
  =>
  (retract ?r)
  (assert(remove (no $?pno ?no)))
  (printout t ?no " ")
)

;; 確認所有候選人是全被標記移除
(defrule check-if-all-candidates-removed
  ...
  (not 
    (and 
        (and 
            (remove (no $? ?n $?)) 
            (candidate (no ?n) (votes ?v))
        ) 
        (test (> ?v 0))
    )
  )
  =>
  (assert (phase print-no-result))
)
